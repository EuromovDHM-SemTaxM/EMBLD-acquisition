...
def __process_atomic_action(action, local_actions, mod: str = None):
    ...
    if type == "atomic":
        if "modifiers" in action:
            for variant in action["modifiers"]:
                ...
                instr_arguments = __extract_instruction_arguments(
                ...
                for inst_arg in instr_arguments:
                    if inst_arg == "{mod}":
                        ...
                        if rand_match:
                            param_value = str(
                                randint(
                                    int(rand_match.group(1)), int(rand_match.group(2))
                                )
                            )
                        else:
                            if len(param_field) == 1:
                                param_value = APP_PARAMETERS[arg_key][param_field[0]][
                                    "label"
                                ]
                            else:
                                param_value = ",".join(
